...
void palm_bld_data(int *turbine_id, int *numbld, int *numbldelem,
                   double *rotrad);
void palm_sim_env(int *turbine_id, double *dtfast,
                  double *shaft_height_fast_in);
void palm_turb_par(int *turbine_id, double *simtimef, double *vel_rot,
                   double *xs_x, double *xs_y, double *xs_z);
void palm_blade_pitch(int *turbine_id, int *blade_id, double *pitch);
void palm_force_table_bld(int *turbine_id, int *max_blade_elem_id,
                          int *blade_elem_id, double *force_x, double *force_y,
                          double *force_z, int *err);
void palm_position_table(int *turbine_id, int *max_blade_elem_id,
                         int *blade_elem_id, double *pos_x, double *pos_y,
                         double *pos_z, int *err);
void palm_vel_value(int *turbine_id, int *bladeid, int *elemid, double *compu,
                    double *compv, double *compw, int *err);
// near-wake-correction (NWC) blade geometry + polars received from OpenFAST
void palm_nwc_set_counts(int *turbine_id, int *nnodes, int *nairfoils,
                         int *max_nrows);
void palm_nwc_set_node(int *turbine_id, int *node_id, double *r_cp,
                       double *twist_deg, double *chord, int *blafid);
void palm_nwc_set_polar_len(int *turbine_id, int *airfoil_id, int *nrows);
void palm_nwc_set_polar_row(int *turbine_id, int *airfoil_id, int *row_id,
                            double *alpha_deg, double *cl, double *cd);
void palm_nwc_set_done(int *turbine_id);
...
double simturbonp;
double simdurationp;
// 1 when PALM wants the near-wake-correction blade geometry + polars appended to
// the connection-init response. Set from the namelist via set_nwc_enabled before
// the connection is initialised; default 0 keeps the connection-init message and
// every non-NWC run unchanged.
int palm_nwc_enabled_client = 0;

// Every turbine connection shares one set of near-wake receive buffers (all
// turbines send the same blade). communication() runs one commfunc thread per
// turbine, so the geometry parse in analyse_msg is serialised with this mutex so
// concurrent threads cannot double-allocate or interleave writes into them.
static pthread_mutex_t nwc_recv_mutex = PTHREAD_MUTEX_INITIALIZER;
int msgcp = 1;
double *adtfast;
double *atfast;
double *rhoairfast;
...
int time_data(double simulated_time_palm, double curdt, int curstep,
              double turbon, double endtime) {

  // sprintf(command, "echo \"%s%f:%f:%d:%f:%f\" >> DEBUG_CCODE", "Time PALM =
  // ", simulated_time_palm, curdt, curstep, turbon, endtime); system(command);
  ...
  if (CMSGOUT > 0) {
    debug_msg("[Debug] time_data: palm_time_since_begin = %f, active_dt = %f, "
              "active_dt_source = %s, cur_step = %d, turbine_on_time = %f, "
              "end_time = %f",
              simtimep, simdtp,
              (simtimep < simturbonp) ? "PALM dt_3d" : "FAST dt_fast", simstepp,
              simturbonp, simendtimep);
  }
...
//---------------------------------------------------------------------------------------------------------------------------------
// record whether PALM wants the near-wake-correction blade geometry + polars in
// the connection-init response. Called once from the coupler (f8c_init) before
// the connection is initialised.
//----------------------------------------------------------
int set_nwc_enabled(int flag) {
  palm_nwc_enabled_client = flag;
  return 0;
}
...
int analyse_msg(unsigned char *velmsg, int ccid) {
  ...
  // at least a header (start_signal, turbine id, message counter, message id)
  // should be present
  ...
  if (turbine_id_f != ccidpone) {
    error_msg("[Error] ID of responding turbine (%d) does not match intended "
              "addressee (%d).",
              turbine_id_f, ccidpone);
    return -1;
  }
  ...
  cbyte = cbyte + 4;
  if (msgcf != msgcp) {
    error_msg("[Error] Synchronization problem between FAST and PALM! Message "
              "counter mismatch (%d:%d).",
              msgcf, msgcp);
    return -1;
  }
  ...
  if (CMSGOUT > 0) {
    // S. ouchene: fixed %d to %f for simtimepf, and explicitly add what
    // variables are output in the Header
    debug_msg("[Debug] Timep: %f", simtimep);
    debug_msg("[Debug] Timestepp: %d", simstepp);
    debug_msg("[Debug] Header: turbine_id_f = %d: msgcf = %d: msgtypef = %d",
              turbine_id_f, msgcf, msgtypef);
  }
  ...
  if ((msgtypef == -1) || ((msgtypef > 0) && (msgtypef < 6))) {
    ...
    if (msgtypef == -1) {
      ...
      // 60 bytes (52 + 8) is the standard connection response. When PALM asked
      // for the near-wake-correction geometry, OpenFAST appends a variable-length
      // geometry + polar block after the shaft height, so in that case only the
      // 60-byte header is guaranteed.
      if ((!palm_nwc_enabled_client && bytein_count[ccid] != 52 + 8) ||
          (palm_nwc_enabled_client && bytein_count[ccid] < 52 + 8)) {
        error_msg("[Error] Unexpected number of bytes received %d:%d.",
                  bytein_count[ccid], (ibldelemp * 3 + 5) * 4 + 8);
        return -1;
      }
      ...
      palm_bld_data(&ccidpone, &anumbld[ccid], &anumbldelem[ccid],
                    &aradturb[ccid]);
      ...
      // Near-wake-correction blade geometry + polars, appended by OpenFAST only
      // when PALM requested it. The block is self-describing (counts first), so
      // parse it and hand it to the coupler via the setters; the coupler builds
      // its NWC arrays from this at the first coupling step.
      if (palm_nwc_enabled_client && bytein_count[ccid] > 52 + 8) {
        int nnodes, nairfoils, max_nrows;
        int n, a, rr, idx, rid, afid, nr;
        double rcp, tw, ch, al, cl, cd;

        // Serialise: all turbine threads fill the same shared receive buffers.
        pthread_mutex_lock(&nwc_recv_mutex);
        nnodes = biconv(velmsg, cbyte);
        cbyte = cbyte + 4;
        nairfoils = biconv(velmsg, cbyte);
        cbyte = cbyte + 4;
        max_nrows = biconv(velmsg, cbyte);
        cbyte = cbyte + 4;
        palm_nwc_set_counts(&ccidpone, &nnodes, &nairfoils, &max_nrows);

        for (n = 0; n < nnodes; n++) {
          rcp = bfconv(velmsg, cbyte);
          cbyte = cbyte + 8;
          tw = bfconv(velmsg, cbyte);
          cbyte = cbyte + 8;
          ch = bfconv(velmsg, cbyte);
          cbyte = cbyte + 8;
          afid = biconv(velmsg, cbyte);
          cbyte = cbyte + 4;
          idx = n + 1;
          palm_nwc_set_node(&ccidpone, &idx, &rcp, &tw, &ch, &afid);
        }
        for (a = 0; a < nairfoils; a++) {
          nr = biconv(velmsg, cbyte);
          cbyte = cbyte + 4;
          idx = a + 1;
          palm_nwc_set_polar_len(&ccidpone, &idx, &nr);
          for (rr = 0; rr < nr; rr++) {
            al = bfconv(velmsg, cbyte);
            cbyte = cbyte + 8;
            cl = bfconv(velmsg, cbyte);
            cbyte = cbyte + 8;
            cd = bfconv(velmsg, cbyte);
            cbyte = cbyte + 8;
            rid = rr + 1;
            palm_nwc_set_polar_row(&ccidpone, &idx, &rid, &al, &cl, &cd);
          }
        }
        palm_nwc_set_done(&ccidpone);
        pthread_mutex_unlock(&nwc_recv_mutex);
      }
      ...
      if (bytein_count[ccid] != ((ibldelemp * 3) * 8 + 20)) {
        error_msg("[Error] Unexpected number of bytes received %d:%d.",
                  bytein_count[ccid],
                  ((ibldelemp * 3) * 8 + 20)); //(ibldelemp*3+5)*4);
        return -1;
      }
      ...
      // position and forces
      // check length of message
      if (bytein_count[ccid] !=
          ((((ibldelemp - 1) * 2 + 1) * 3 * 8) + 20 + 8 + 24 + 8 +
           anumbld[ccid] * 8)) {
        error_msg("[Error] Unexpected number of bytes received %d:%d.",
                  bytein_count[ccid],
                  ((((ibldelemp - 1) * 2 + 1) * 3 * 8) + 20 + 8 + 24 + 8 +
  ...
  if ((msgtypef == 3) || (msgtypef == 4)) {
    ...
    while (cpack < ibldelemp) {
      ...
      palm_position_table(&ccidpone, &ibldelemp, (int *)&cpack, &bepos[0],
                          &bepos[1], &bepos[2], &err);
      if (CMSGOUT > 0) {
        debug_msg("[Debug] positions: %d %d %d %f %f %f", ccidpone, ibldelemp,
                  cpack, bepos[0], bepos[1], bepos[2]);
      }

      if (err != 0) {
        error_msg("[Error] Unable to update blade element positions in PALM "
                  "(id: %d).",
                  ccidpone);
        return -1;
      }
  ...
  if (msgtypef == 4) {
    ...
    while (cpack < ibldelemp - 1) {
      ...
      palm_force_table_bld(&ccidpone, &ibldelemp, (int *)&cpack, &befor[0],
                           &befor[1], &befor[2], &err);
      if (CMSGOUT > 0) {
        debug_msg("[Debug] forces:%d %d %d %f %f %f ", ccidpone, ibldelemp,
                  cpack, befor[0], befor[1], befor[2]);
      }

      if (err != 0) {
        error_msg("[Error] Unable to update forces at blade elements in PALM "
                  "(id: %d).",
                  ccidpone);
        return -1;
      }
    ...
    cbyte = cbyte + 8;
    txs_z = xs_z[ccid];
    status_msg("[Status] [xs] %f, %f, %f", txs_x, txs_y, txs_z);

    // blade pitch (radians), one value per blade, in the same order the
    // OpenFAST side packs them (after the shaft vector)
    {
      int bpk;
      int bpid;
      double tpitch;
      for (bpk = 0; bpk < anumbld[ccid]; bpk++) {
        tpitch = bfconv(velmsg, cbyte);
        cbyte = cbyte + 8;
        bpid = bpk + 1;
        palm_blade_pitch(&ccidpone, &bpid, &tpitch);
      }
    }

    palm_turb_par(
        &ccidpone, &tsimtimef, &tvel_rot, &txs_x, &txs_y,
        &txs_z); // sending FAST rotation velocity to fortran part of PALM
  }
  ...
  */
  ...
  if ((msgtype > 0) || (msgtype < 7)) {
    ...
    if (msgtype == 1) {
      ...
      // near-wake-correction request flag. Appended only when PALM wants the
      // blade geometry + polars in OpenFAST's connection response. With the
      // correction off no flag is sent, so the message stays the original
      // 48-byte (plus end marker) connection init and an OpenFAST build that
      // predates the correction still accepts it.
      if (palm_nwc_enabled_client) {
        ival = palm_nwc_enabled_client;
        ibconv(msg2f, ival, cval);
        cval = cval + 4;
      }
      ...
      if (CMSGOUT > 0) {
        debug_msg("[Debug] Sending 3*%d velocities to turbine %d, timestep: .",
                  simstepp, anumbld[ccid] * anumbldelem[ccid] + 1, ccidpone);
      }
      ...
      for (j = 0; j < anumbld[ccid]; j++) {
        ...
        for (i = 0; i < anumbldelem[ccid]; i++) {
          cbld = j + 1;
          cbldelem = i + 1;
          palm_vel_value(&ccidpone, &cbld, &cbldelem, &velu, &velv, &velw,
                         &err);
          if (err != 0) {
            error_msg("[Error] Unable to retrieve PALM velocities (id: %d).",
                      ccidpone);
            return -1;
          }
          debug_msg("[Debug] velocities: [%d][%d] %f:%f:%f", cbld, cbldelem,
                    velu, velv, velw);
      ...
      debug_msg("[Debug] hub velocity: [%d][%d] %f:%f:%f", cbld, cbldelem, velu,
                velv, velw);
...
void *commfunc(void *param) {
  ...
  if (CMSGOUT > 0) {
    debug_msg("[Debug] PALM client sending data to: %s:%s (mc = %d)",
              host_fast[ccid], port_fast[ccid], msgcp);
  ...
  while (totalsent[ccid] < 4) {
    ...
    debug_msg("[Debug] To be sent to PALM A: 4");
    debug_msg("[Debug] Sent to PALM A: %d", totalsent[ccid]);
  ...
  while (totalsent[ccid] < byteout_count[ccid]) {
    ...
    stilltosend[ccid] -= cflag;
    loopcounter[ccid] = loopcounter[ccid] + 1;
    debug_msg("[Debug] To be sent to PALM B: %d", byteout_count[ccid]);
    debug_msg("[Debug] Sent to PALM B: %d", totalsent[ccid]);
  ...
  if (cflag != 4) {
    error_msg("[Error] Unable to read msg length from socket (id: %d).",
              ccid + 1);
  ...
  if (cflag < 0) {
    ...
    status_msg(
        "[Status] At least one FAST instance finished the simulation (id: %d).",
        ccid + 1);
    retptr[ccid] = 1;
    pthread_exit((void *)&retptr[ccid]);
  } else if (cflag == 2) {
    error_msg(
        "[Error] An error in at least one FAST instance occurred (id: %d).",
        ccid + 1);
    retptr[ccid] = -1;
    pthread_exit((void *)&retptr[ccid]);
  }
...
int commclient(int target, int ccmsgid) {
  ...
  //  Note that FAST is defined above: FAST=0, PALM=1
  if (target == FAST) {
    ...
    for (iturb = 0; iturb < inumturb; iturb++) {
      ...
      *tnum = iturb;
      ...
      if (err != 0) {
        error_msg("[Error] Unable to create client thread [%s]!",
                  strerror(err));
        clean_exit();
        return -1;
      } else {
        status_msg("[Status] Client thread %d for communication with %s:%s "
                   "running (mc=%d)...",
                   iturb, host_fast[iturb], port_fast[iturb], msgcp);
      }
    }
    ...
    for (iturb = 0; iturb < inumturb; iturb++) {
      ...
      if (retptr[iturb] == 1) {
        status_msg(
            "[Status] At least one FAST instance finished the simulation!");
        clean_exit();
        return 1;
      }
    }
    ...
    if (msgcp == 1) {
      for (iturb = 0; iturb < inumturb; iturb++) {
        ...
        if (rhoairfast[iturb] != rhoairfast[0]) {
          error_msg(
              "[Error] All FAST instances must run with the same air density.");
          clean_exit();
          return 1;
        }
      }
      ...
      for (iturb = 0; iturb < inumturb; iturb++) {
        ...
        palm_sim_env(
            &ccidpone, &tdtfast,
            &shaft_height_fast_tmp); // sending FAST time step and Shaft height
                                     // to fortran part of PALM
      }
    }
