...
!--------------------------------------------------------------------------------------------------!
 MODULE user
    ...
    USE fastv8_coupler_mod, only:current_time_fast, rotspeed, &
                            shaft_coordinates, first_restart_with_wt, palm_time_at_begin
    
    USE nw_smearing_correction_module, only: nw_X_history,nw_Y_history, & 
                                       nw_vn_history, nw_vt_history, nw_Gamma_history, &
                                       nw_dyn_comp_history, nw_W_history
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE user_rrd_global_ftn( found )
    ...
    PRINT*, 'Reading user restart data'

    SELECT CASE ( restart_string(1:length) )

       CASE ( 'palm_time_at_begin' )
          READ ( 13 )  palm_time_at_begin
       CASE ( 'first_restart_with_wt' )
          READ ( 13 )  first_restart_with_wt

!
!--    The near-wake correction history arrays are allocated in f8c_parin (which runs before the
!--    restart data are read) only when use_near_wake_smearing_correction is enabled. If the file
!--    contains them but the correction is disabled in this run, consume the data record without
!--    reading it; reading into the unallocated arrays would crash.
       CASE ( 'nw_X_history' )
          IF ( ALLOCATED( nw_X_history ) )  THEN
             READ ( 13 )  nw_X_history
          ELSE
             READ ( 13 )
          ENDIF
       CASE ( 'nw_Y_history' )
          IF ( ALLOCATED( nw_Y_history ) )  THEN
             READ ( 13 )  nw_Y_history
          ELSE
             READ ( 13 )
          ENDIF
       CASE ( 'nw_vn_history' )
          IF ( ALLOCATED( nw_vn_history ) )  THEN
             READ ( 13 )  nw_vn_history
          ELSE
             READ ( 13 )
          ENDIF
       CASE ( 'nw_vt_history' )
          IF ( ALLOCATED( nw_vt_history ) )  THEN
             READ ( 13 )  nw_vt_history
          ELSE
             READ ( 13 )
          ENDIF
       CASE ( 'nw_Gamma_history' )
          IF ( ALLOCATED( nw_Gamma_history ) )  THEN
             READ ( 13 )  nw_Gamma_history
          ELSE
             READ ( 13 )
          ENDIF
       CASE ( 'nw_dyn_comp_history' )
          IF ( ALLOCATED( nw_dyn_comp_history ) )  THEN
             READ ( 13 )  nw_dyn_comp_history
          ELSE
             READ ( 13 )
          ENDIF
       CASE ( 'nw_W_history' )
          IF ( ALLOCATED( nw_W_history ) )  THEN
             READ ( 13 )  nw_W_history
          ELSE
             READ ( 13 )
          ENDIF

       CASE ( 'current_time_fast' )
          READ ( 13 )  current_time_fast    
       CASE ( 'rotspeed' )
          READ ( 13 )  rotspeed           
       CASE ( 'shaft_coordinates' )
          READ ( 13 )  shaft_coordinates
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE user_rrd_global_mpi
    ...
    USE restart_data_mpi_io_mod,                                                                   &
        ONLY:  rrd_mpi_io_global_array, rrd_mpi_io


    CALL rrd_mpi_io( 'palm_time_at_begin', palm_time_at_begin )
    CALL rrd_mpi_io( 'first_restart_with_wt', first_restart_with_wt )

!
!-- The near-wake correction history arrays are allocated in f8c_parin (which runs before the
!-- restart data are read) only when use_near_wake_smearing_correction is enabled. Runs with the
!-- correction disabled do not write them (see user_wrd_global), so they must not be read here
!-- either. Passing the unallocated arrays to rrd_mpi_io_global_array would hand null buffers to
!-- MPI-IO and crash. All seven arrays are allocated together, so checking one of them is enough.
    IF ( ALLOCATED( nw_X_history ) )  THEN
       CALL rrd_mpi_io_global_array( 'nw_X_history', nw_X_history )
       CALL rrd_mpi_io_global_array( 'nw_Y_history', nw_Y_history )
       CALL rrd_mpi_io_global_array( 'nw_vn_history', nw_vn_history )
       CALL rrd_mpi_io_global_array( 'nw_vt_history', nw_vt_history )
       CALL rrd_mpi_io_global_array( 'nw_Gamma_history', nw_Gamma_history )
       CALL rrd_mpi_io_global_array( 'nw_dyn_comp_history', nw_dyn_comp_history )
       CALL rrd_mpi_io_global_array( 'nw_W_history', nw_W_history )
    ENDIF

    CALL rrd_mpi_io_global_array( 'current_time_fast', current_time_fast )
    CALL rrd_mpi_io_global_array( 'rotspeed', rotspeed )
    CALL rrd_mpi_io_global_array( 'shaft_coordinates', shaft_coordinates )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE user_wrd_global
    ...
    USE restart_data_mpi_io_mod,                                                                   &
        ONLY:  wrd_mpi_io_global_array, wrd_mpi_io


    IF ( TRIM( restart_data_format_output ) == 'fortran_binary' )  THEN
    
    PRINT*, 'Writing user restart data'

       CALL wrd_write_string( 'palm_time_at_begin' )
       WRITE ( 14 )  palm_time_at_begin
       CALL wrd_write_string( 'first_restart_with_wt' )
       WRITE ( 14 )  first_restart_with_wt

!
!--    Write the near-wake correction history only when its arrays exist. They are allocated in
!--    f8c_parin only when use_near_wake_smearing_correction is enabled; writing them unallocated
!--    would crash. The readers skip these entries under the same condition, which keeps write and
!--    read symmetric. All seven arrays are allocated together, so checking one of them is enough.
       IF ( ALLOCATED( nw_X_history ) )  THEN
          CALL wrd_write_string( 'nw_X_history' )
          WRITE ( 14 )  nw_X_history
          CALL wrd_write_string( 'nw_Y_history' )
          WRITE ( 14 )  nw_Y_history
          CALL wrd_write_string( 'nw_vn_history' )
          WRITE ( 14 )  nw_vn_history
          CALL wrd_write_string( 'nw_vt_history' )
          WRITE ( 14 )  nw_vt_history
          CALL wrd_write_string( 'nw_Gamma_history' )
          WRITE ( 14 )  nw_Gamma_history
          CALL wrd_write_string( 'nw_dyn_comp_history' )
          WRITE ( 14 )  nw_dyn_comp_history
          CALL wrd_write_string( 'nw_W_history' )
          WRITE ( 14 )  nw_W_history
       ENDIF

       CALL wrd_write_string( 'current_time_fast' )
       WRITE ( 14 )  current_time_fast  
       CALL wrd_write_string( 'rotspeed' )
       WRITE ( 14 )  rotspeed
       CALL wrd_write_string( 'shaft_coordinates' )
       WRITE ( 14 )  shaft_coordinates
       

    ELSEIF ( restart_data_format_output(1:3) == 'mpi' )  THEN
    
    PRINT*, 'Writing mpi restart data'

    CALL wrd_mpi_io( 'palm_time_at_begin', palm_time_at_begin )
    CALL wrd_mpi_io( 'first_restart_with_wt', first_restart_with_wt )

!
!-- Write the near-wake correction history only when its arrays exist. They are allocated in
!-- f8c_parin only when use_near_wake_smearing_correction is enabled. Writing them unallocated
!-- registers header entries with null data buffers, and the continuation run crashes when it
!-- reads them back. user_rrd_global_mpi skips these entries under the same condition.
    IF ( ALLOCATED( nw_X_history ) )  THEN
       CALL wrd_mpi_io_global_array( 'nw_X_history', nw_X_history )
       CALL wrd_mpi_io_global_array( 'nw_Y_history', nw_Y_history )
       CALL wrd_mpi_io_global_array( 'nw_vn_history', nw_vn_history )
       CALL wrd_mpi_io_global_array( 'nw_vt_history', nw_vt_history )
       CALL wrd_mpi_io_global_array( 'nw_Gamma_history', nw_Gamma_history )
       CALL wrd_mpi_io_global_array( 'nw_dyn_comp_history', nw_dyn_comp_history )
       CALL wrd_mpi_io_global_array( 'nw_W_history', nw_W_history )
    ENDIF

    CALL wrd_mpi_io_global_array( 'current_time_fast', current_time_fast )
    CALL wrd_mpi_io_global_array( 'rotspeed', rotspeed )
    CALL wrd_mpi_io_global_array( 'shaft_coordinates', shaft_coordinates )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE user_last_actions
    ...
    USE wt_output_mod,                                                                             &
        ONLY:  wt_output_finalize

!
!-- Close the wind-turbine NetCDF output file (if it was opened). No-op on non-rank-0 and
!-- when the output was never enabled (output_interval = 0 in fastv8_coupler_parameters).
    CALL wt_output_finalize
